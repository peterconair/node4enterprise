...
var rabit = require("crypto-js/rabbit");
 var encrypted = rabit.encrypt("Message", "Secret Passphrase");
 console.log('en',encrypted.toString());

 //var decrypted = rabit.decrypt(encrypted, "Secret Passphrase");
 //console.log('de',decrypted);

 var bytes  = rabit.decrypt(encrypted, 'secret key 123');
//var plaintext = bytes.toString(rabit.enc.Utf8);
//var decryptedData = JSON.parse(bytes.toString(CryptoJS.enc.Utf8));
console.log('de',bytes.toString());*/

/*var CryptoJS = require("crypto-js");

var data = [{name:"admin",password:"p@ssw0rd"}, {name: "reader",password:"reader"}];

// Encrypt
var ciphertext = CryptoJS.Rabbit.encrypt(JSON.stringify(data), 'secret key 123');
console.log(ciphertext.toString());
// Decrypt
var bytes  = CryptoJS.Rabbit.decrypt(ciphertext.toString(), 'secret key 123');
var decryptedData = JSON.parse(bytes.toString(CryptoJS.enc.Utf8));

console.log(decryptedData);*/
//var swagger = require('swagger-express');
//var fs = require('fs'),path = require('path');
//	var dirs=	getDirectories('docs');
  //console.log(dirs);
/*var dirTree = require('directory-tree');
var tree = dirTree.directoryTree('docs');
console.log(JSON.stringify(tree));*/
/*var doc = require('api/docs/docManager');
doc = new doc();
var tree = doc.getModules();
*/

var config = require('config');
var CryptoJS = require("crypto-js");
var bytes  = CryptoJS.Rabbit.decrypt(config.API_USERS, 'secret key 123');
var userApis= JSON.parse(bytes.toString(CryptoJS.enc.Utf8));
//console.log(decryptedData);
userApis.forEach(function(a){
  users.insert(a);
})
